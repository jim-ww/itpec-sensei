// Package repository defines the progress store's persistence boundary: the
// Repository interface plus the request/row types it speaks in. Business
// logic (grading, scoping, stats aggregation, session resume/repeat) lives in
// package core and depends only on this interface, not on any concrete
// database — see package sqlite for the real implementation, and swap in a
// fake Repository to unit test core's logic without a database at all.
package repository
...
// statement core.Core needs lives behind this interface.
