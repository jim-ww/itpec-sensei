...
func (c *Core) GetProgressSummary(ctx context.Context, scope Scope, period Period) (*ProgressSummary, error) {
	...
	filter := repository.AttemptFilter{QuestionIDs: questionIDList(ids)}
...
func (c *Core) GetTopicStats(ctx context.Context, scope Scope) ([]TopicStat, error) {
	...
	rows, err := c.Repo.ListAttempts(ctx, repository.AttemptFilter{QuestionIDs: questionIDList(ids)}, "", 0)
	if err != nil {
		return nil, err
	}
