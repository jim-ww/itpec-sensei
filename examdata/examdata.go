// Package examdata is the wire format of the question bank (one JSON file
// per exam), loaded by core into a Bank — the single source of truth for
// that JSON schema.
package examdata
...
// SubAnswers accordingly. Called once per question before it's written to
// disk, so that by the time a Question reaches disk (and later, a Bank),
// Answer's polymorphism has already been resolved.
...
// explanation (and therefore no topic) is available for it.
...
// ExamData is everything known about one exam — the top-level shape of one
// exam JSON file.
type ExamData struct {
	ExamID      string     `json:"examId"`
	DataVersion int        `json:"scraperVersion"`
