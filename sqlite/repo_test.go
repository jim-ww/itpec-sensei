...
func TestOpenAppliesEmbeddedSchema(t *testing.T) {
	repo := newTestRepo(t)
	id, err := repo.InsertSession(context.Background(), core.SessionParams{ExamType: "fe", Mode: "normal", OrderStrategy: "random"})
	require.NoError(t, err)
	assert.NotZero(t, id)
}
...
func TestInsertAndListAttempts(t *testing.T) {
	...
	sessionA, err := repo.InsertSession(ctx, core.SessionParams{ExamType: "fe", Mode: "normal", OrderStrategy: "random"})
	require.NoError(t, err)
	sessionB, err := repo.InsertSession(ctx, core.SessionParams{ExamType: "fe", Mode: "normal", OrderStrategy: "random"})
	require.NoError(t, err)
	...
	tests := []struct {
		name    string
		filter  core.AttemptFilter
		order   core.HistoryOrder
		limit   int
		wantIDs []string // question ids, in expected order
	}{
		{
			name:    "filters by session",
			filter:  core.AttemptFilter{SessionID: sessionA},
			order:   core.HistoryNewestFirst,
			wantIDs: []string{"examA#2", "examA#1"},
		},
		{
			name:    "filters by question ids",
			filter:  core.AttemptFilter{QuestionIDs: []string{"examA#1", "examB#1"}},
			order:   core.HistoryOldestFirst,
			wantIDs: []string{"examA#1", "examB#1"},
		},
		{
			name:    "empty non-nil question id filter matches nothing",
			filter:  core.AttemptFilter{QuestionIDs: []string{}},
			order:   core.HistoryNewestFirst,
			wantIDs: nil,
		},
		{
		...
		},
		{
			...
			filter:  core.AttemptFilter{Since: ptrTime(t0.Add(90 * time.Minute))},
			order:   core.HistoryNewestFirst,
			wantIDs: []string{"examB#1"},
		},
		{
			name:    "question ids combined with since",
			filter:  core.AttemptFilter{QuestionIDs: []string{"examA#1", "examA#2"}, Since: ptrTime(t0.Add(30 * time.Minute))},
			order:   core.HistoryNewestFirst,
...
func TestDeleteAttempt(t *testing.T) {
	repo := newTestRepo(t)
	ctx := context.Background()
	sessionID, err := repo.InsertSession(ctx, core.SessionParams{ExamType: "fe", Mode: "normal", OrderStrategy: "random"})
	require.NoError(t, err)
	require.NoError(t, repo.InsertAttempt(ctx, sessionID, "examA#1", "A", true, false, 0, time.Now().UTC()))

	rows, err := repo.ListAttempts(ctx, core.AttemptFilter{SessionID: sessionID}, "", 0)
	require.NoError(t, err)
...
func TestQuestionSRS(t *testing.T) {
	...
	require.NoError(t, repo.UpsertQuestionSRS(ctx, "examA#1", core.QuestionSRS{
		Box: 1, DueAt: t0.Add(24 * time.Hour), LastReviewedAt: t0,
	}))
	// examA#2 was due yesterday -> due as of t0.
	require.NoError(t, repo.UpsertQuestionSRS(ctx, "examA#2", core.QuestionSRS{
...
func TestSessionLifecycle(t *testing.T) {
	...
	sessions, err := repo.ListSessions(ctx, core.HistoryNewestFirst)
	require.NoError(t, err)
...
func TestListSessionsAggregatesAnsweredAndCorrect(t *testing.T) {
	...
	s1, err := repo.InsertSession(ctx, core.SessionParams{ExamType: "fe", Mode: "normal", OrderStrategy: "random"})
	require.NoError(t, err)
	time.Sleep(time.Millisecond) // ensure a distinct started_at ordering
	s2, err := repo.InsertSession(ctx, core.SessionParams{ExamType: "fe", Mode: "normal", OrderStrategy: "random"})
	require.NoError(t, err)
	...
	sessions, err := repo.ListSessions(ctx, core.HistoryOldestFirst)
	require.NoError(t, err)
	require.Len(t, sessions, 2)

	byID := map[int64]core.SessionRecord{}
...
func TestDeleteSession(t *testing.T) {
	...
	id, err := repo.InsertSession(ctx, core.SessionParams{ExamType: "fe", Mode: "normal", OrderStrategy: "random"})
	require.NoError(t, err)
	...
	rows, err := repo.ListAttempts(ctx, core.AttemptFilter{SessionID: id}, "", 0)
	require.NoError(t, err)
...
func TestResetAllProgress(t *testing.T) {
	...
	rows, err := repo.ListAttempts(ctx, core.AttemptFilter{}, "", 0)
	require.NoError(t, err)
	assert.Empty(t, rows)
}
